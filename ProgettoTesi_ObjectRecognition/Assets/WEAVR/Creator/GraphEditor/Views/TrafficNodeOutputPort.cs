...
{
    class TrafficNodeOutputPort : FlowPort, IEdgeConnectorListener, IControlledElement, ISettableControlledElement<TrafficNodePortController>, IMasterPort
    {
        private TrafficNodePortController m_controller;

        public TrafficNodePortController Controller
        {
            get => m_controller;
            set
            {
        ...
        protected TrafficNodeOutputPort(Orientation portOrientation, Direction portDirection, Capacity portCapacity) : base(portOrientation, portDirection, portCapacity)
        {
            AddToClassList("conditionPort");
        }

        public static TrafficNodeOutputPort Create(TrafficNodePortController controller, Orientation orientation = Orientation.Horizontal, Capacity capacity = Capacity.Multi)
        {
            var flowPort = new TrafficNodeOutputPort(orientation, Direction.Output, capacity);
