...
namespace TXT.WEAVR.Controls
{
    ...
    public class AdvancedDropdownWindow : EditorWindow
    {
        private static class Styles
        {
            ...
            public static GUIStyle descriptionText = WeavrStyles.EditorSkin2.FindStyle("advancedPopup_Description") 
        ...
        protected AdvancedDropdownGUI m_gui = null;
        public AdvancedDropdownDataSource m_dataSource = null;
        ...
        private bool hasSearch { get { return !string.IsNullOrEmpty(m_searchKeyword); } }
        ...
        public static T CreateAndInit<T>(Rect rect, AdvancedDropdownDataSource dataSource) where T : AdvancedDropdownWindow
        {
            var instance = CreateInstance<T>();
            instance.Init(rect, dataSource);
            return instance;
        }
        ...
        public void Init(Rect buttonRect, AdvancedDropdownDataSource dataSource = null)
        {
            m_ButtonRectScreenPos = GUIToScreenRect(buttonRect);
            m_dataSource = dataSource ?? m_dataSource ?? new MultiLevelDataSource();
            if (m_gui == null)
            {
                m_gui = new AdvancedDropdownGUI(m_dataSource);
            }
            selectionChanged += m_dataSource.UpdateSelectedId;
        ...
        private void OnDirtyList()
        {
            ...
            if (hasSearch)
                m_dataSource.RebuildSearch(m_searchKeyword);
        }

        private void OnGUISearch()
        {
            m_gui.DrawSearchField(isSearchFieldDisabled, m_searchKeyword, (newSearch) =>
            {
                m_dataSource.RebuildSearch(newSearch);
                m_CurrentlyRenderedTree =
                    string.IsNullOrEmpty(newSearch) ? m_dataSource.mainTree : m_dataSource.searchTree;
                m_searchKeyword = newSearch;
                if (!string.IsNullOrEmpty(newSearch))
                {
                    m_CurrentlyRenderedTree.selectedItem = 0;
                }
            });
        }
        ...
        private void DrawDropdown(float anim, AdvancedDropdownItem group)
        {
            ...
            if (group.GetSelectedChild() is IHasDescription descriptable && descriptable.Description != null)
            {
                description = descriptable.Description;
                m_tempContent.text = description;
                descriptionHeight = Styles.descriptionText.CalcHeight(m_tempContent, position.width - kBorderThickness * 2);
            }

            // Start of animated area (the part that moves left and right)
            var areaPosition = new Rect(0, 0, position.width, position.height - descriptionHeight);
            ...
            // Header
            if (showHeader)
                m_gui.DrawHeader(group, GoToParent);
            ...
            {
                m_tempContent.text = description;
                GUI.Label(new Rect(areaPosition.x,
                                   areaPosition.y + areaPosition.height,
                                   areaPosition.width,
                                   descriptionHeight - kBorderThickness),
        ...
        private void DrawList(AdvancedDropdownItem item)
        {
            ...
            for (var i = 0; i < item.children.Count; i++)
            {
                ...
                m_gui.DrawItem(child, selected, hasSearch);
            ...
            //GUILayout.Label("LSdkbjalskjdb laksbla ksjbla jslb jaslkbdjals blsa kjblajds blkjdsblakjlb ksajlb alsdkb las jbla jblka bdl jdslbj");
        }
