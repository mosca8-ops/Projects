...
public class YOLOHandler : IDisposable
{
    ...
    private List<string> _preprocessorKeywords = new List<string> { "preprocessor", "resizing", "rescaling", "centercrop" };
    private string[] classesNames;
    private int classesNum;
    // float[] anchors = new float[] { 1.08f, 1.19f, 3.42f, 4.41f, 6.63f, 11.38f, 9.42f, 5.11f, 16.62f, 10.52f }; //default
    //float[] anchors = new float[] { 0.57273f, 0.677385f, 1.87446f, 2.06253f, 3.33843f, 5.47434f, 7.88282f, 3.52778f, 9.77052f, 9.16828f }; // from .cfg
    //float[] anchors = new float[] {4.31f,3.50f, 4.55f,5.31f, 6.35f,6.11f, 7.75f,5.05f, 8.47f,7.83f }; // create sul mio dataset
    ...
    public YOLOHandler(NNHandler nn, string[] classesNames)
    {
        this.nn = nn;
        this.classesNames = classesNames;
        this.classesNum = classesNames.Length;
        this.anchors = nn.anchors;
        
        ops = BarracudaUtils.CreateOps(WorkerFactory.Type.Compute);
        cpuOps = BarracudaUtils.CreateOps(WorkerFactory.Type.CSharp);
        inputWidthHeight = nn.model.inputs[0].shape[BarracudaUtils.WidthIndex];
        // check if the model has preprocessed
        premulTensor = new Tensor(1, 1, new float[] { ContainsPreprocessing(nn.model) ? 255 : 1 });
        PerformanceCounter.GetInstance()?.AddCounter(stopwatch);
    }
    private bool ContainsPreprocessing(Model model)
    {
        for (int i = 0; i < model.layers.Count; i++)
        {
            for (int j = 0; j < _preprocessorKeywords.Count; j++)
            {
                if (model.layers[i].name.ToLower().Contains(_preprocessorKeywords[j].ToLower()))
                {
                    return true;
                }
            }
    ...
    public async Task<List<ResultBox>> RunAsync(Texture2D tex)
    {
        //Profiler.BeginSample("YOLO.RunAsync");
        stopwatch.Start();
        ...
        //Profiler.BeginSample("YOLO.RunAsync.2");

        Tensor output = await ExecuteAsync(preprocessed, nn.worker); //.ConfigureAwait(false);
        //Tensor output = await Task.Run(() => ExecuteAsync(preprocessed, nn.worker));

        //Profiler.EndSample();

        preprocessed.Dispose();
        var results = await PostprocessAsync(output); //.ConfigureAwait(false);
    ...
    public async Task<Tensor> ExecuteAsync(Tensor preprocessed, IWorker worker)
    {
        //Profiler.BeginSample("YOLO.ExecuteAsync");
        stopwatch.Start();
        //Profiler.BeginSample("YOLO.ExecuteAsync.Pre");

        var executor = worker.StartManualSchedule(preprocessed);
        var it = 0;
        bool hasMoreWork;

        //Profiler.EndSample();
        //Profiler.BeginSample("YOLO.ExecuteAsync.Post");
        ...
        stopwatch.Stop();
        //Profiler.EndSample();
        //Debug.Log("ExecuteAsync: " + stopwatch.Value);

        return worker.CopyOutput();
    }

    private async Task<List<ResultBox>> PostprocessAsync(Tensor x)
    {
        //Profiler.BeginSample("YOLO.PostprocessAsync");
        stopwatch.Start();
        ...
        stopwatch.Stop();
        //Profiler.EndSample();
        //Debug.Log("PostprocessAsync: " + stopwatch.Value);
    ...
    {
        stopwatch.Start();
        Profiler.BeginSample("YOLO.DecodeNNOut");
        ...
        stopwatch.Stop();
        //Debug.Log("DecodeNNOut: " + stopwatch.Value);
    ...
    {
        //Profiler.BeginSample("YOLO.DecodeBox");
        stopwatch.Start();
        ...
        stopwatch.Stop();
        //Debug.Log("DecodeBox: " + stopwatch.Value);
        //Profiler.EndSample();
    ...
    public List<ResultBox> Run(Texture2D tex)
    {
        ...
        input.Dispose();
        stopwatch.Start();
        Profiler.BeginSample("YOLO.Run.2");
        //Tensor output = ExecuteAsync(preprocessed, nn.worker);
    ...
    /*
    public List<ResultBox> Postprocess(Tensor x)
    {
    ...
    /*
    private async Task<List<ResultBox>> DecodeNNOutAsync(Tensor output)
    {
        Profiler.BeginSample("YOLO.DecodeNNOutAsync");
        float[] data = output.AsFloats();
        Profiler.EndSample();
    ...
    /*
    private async Task<ResultBox?> DecodeBoxAsync(float[] data, int startIndex, int x_cell, int y_cell, int box)
    {
        Profiler.BeginSample("YOLO.DecodeBox");
        float box_score = Sigmoid(data[startIndex + 4]);
        if (box_score < DISCARD_TRESHOLD)
